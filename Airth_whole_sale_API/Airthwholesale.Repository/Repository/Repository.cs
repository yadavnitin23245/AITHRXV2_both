...
namespace Airthwholesale.Repository.Repository
{
    ...
    {
        private readonly AirthwholesaleDbContext _context;
        ...
        public Repository(AirthwholesaleDbContext context, JDPAPIDbContext JDPcontext)
        {
            _context = context;
        ...
        /// <returns></returns>
        /// <exception cref="ArgumentNullException">entity is null</exception>
        public async Task<int?> JDP_InsertAsync(T entity)
        {
            try
            {
                ...
                if (entity == null)
                    throw new ArgumentNullException("entity is null");
                _entities.Add(entity);
                _JDPcontext.Entry(entity).State = EntityState.Added;
                return await _JDPcontext.SaveChangesAsync();
            }
            catch (Exception exception)
            {
                var message = "";
        ...
        public async Task<int?> JDP_InsertMultiAsync(List<T> entity)
        {
            ...
            try
            {
                ...
                {
                    //i used Z.EntityFramework.Extensions.EFCore for bulk insertion and used in repository
                    _JDPcontext.BulkInsert(entity);

                    //entity.ForEach(a => { _JDPcontext.Entry(a).State = EntityState.Added; });   
                    return await _JDPcontext.SaveChangesAsync();
                }
        ...
        //execute with jdp other server or database

        public List<T> ExecuteWithJsonResult_FROM_JDPSERVER(string name, string parserString, params SqlParameter[] parameters)
        {
            var query = _queryBuilder(name, parameters);
            List<T> result;
            var connectionObject = _JDPcontext.Database.GetDbConnection();
            connectionObject.Open();
            using (var command = connectionObject.CreateCommand())
            {
                command.CommandText = query;
